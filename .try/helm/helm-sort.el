...
(define-advice helm-list-directory (:override (directory) add-sort-by-moob)
  (let* ((files (directory-files directory t directory-files-no-dot-files-regexp))
	 (sfiles (pcase helm-ff-sort-method
		   ('nil
		    files)
		   ('newest
...
(define-key helm-find-files-map (kbd "H-q r") #'helm-ff-sort-reverse-toggle)

(define-key helm-find-files-map (kbd "H-q q") #'helm-ff-sort-off)
(define-key helm-find-files-map (kbd "H-q a") #'helm-ff-sort-newer)
(define-key helm-find-files-map (kbd "H-q t") #'helm-ff-sort-type)
(define-key helm-find-files-map (kbd "H-q s") #'helm-ff-sort-biggest)
