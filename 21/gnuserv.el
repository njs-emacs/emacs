...
(defconst gnuserv-rcs-header-id "$Header: gnuserv.el,v 2.1a 98/10/03 14:28:00 saurian $")
...
(defun server-process-filter (proc string)
  ...
  (cond
   ((string-match "\^D$" server-string) ; requests end with ctrl-D
   ...
   ((string-match "Error code:" server-string)
    (progn				;error string from server
      (server-process-display-error server-string)
      (setq server-string "")))
   )
