From - Sun Jul 27 18:43:10 1997
Path: news.demon.co.uk!dispatch.news.demon.net!demon!cam-news-hub1.bbnplanet.com!cpk-news-hub1.bbnplanet.com!news.bbnplanet.com!EU.net!blackbush.xlink.net!news-ge.switch.ch!eru.mt.luth.se!news.kth.se!news.ifm.liu.se!not-for-mail
From: Peter Liljenberg <petli@lysator.liu.se>
Newsgroups: gnu.emacs.sources
Subject: Emacs lisp linter
Date: 27 Jul 1997 10:01:55 +0200
Organization: Lysator Computer Society, =?ISO-8859-1?Q?Link=F6ping?=
 University, Sweden
Sender: petli@tintin.lysator.liu.se
Message-ID: <ywxd8o4j53g.fsf@tintin.lysator.liu.se>
NNTP-Posting-Host: tintin.lysator.liu.se
Mime-Version: 1.0 (generated by tm-edit 7.106)
Content-Type: text/plain; charset=US-ASCII
X-Newsreader: Gnus v5.4.59/Emacs 19.34
Lines: 753
Xref: demon gnu.emacs.sources:1841

Some time back I started to write a HTTP/1.1 elisp interface, and
after a little time realised that I had written five hundred lines of
untested network code... I didn't want to debug that by running the
code, so instead I started writing a linter to catch typos and other
similar bugs. This is the linter, I don't know if I ever bother to
finish that http interface...
...
;; Copyright (C) 1997 Peter Liljenberg

;; $Id: elint.el,v 1.4 1997/07/27 06:21:58 petli Exp $
...
(defun elint-current-buffer ()
  ...
  (elint-clear-log (format "Linting %s" (if (buffer-file-name)
					  ...
					  (buffer-name))))
  (elint-display-log)
  (let* ((forms (elint-get-top-forms))
	 (env (elint-init-env forms)))
    (set (make-local-variable 'elint-buffer-env) env)
    (mapcar 'elint-top-form forms))
...
(defun elint-defun ()
  ...
  (save-excursion
    ...
    (let ((pos (point))
      ...
      (if (not (local-variable-p 'elint-buffer-env))
	  (set (make-local-variable 'elint-buffer-env)
	       (elint-init-env (elint-get-top-forms))))
...
(defun elint-init-env (forms)
  ...
  (let ((env (elint-make-env))
	...
	(let ((name (eval (car (cdr form))))
	  ...
	  (condition-case nil
	      (progn
		;; Load the library, and get its defined symbols
		(eval form)
		(let* ((libname (if (stringp file)
				    file
				  (symbol-name name)))
		       (lib (regexp-assoc libname load-history)))

		  ;; Add all defined variables to the environment
		  (while lib
		    (if (and (symbolp (car lib))
			     (boundp (car lib)))
			(setq env (elint-env-add-var env (car lib))))
		    (setq lib (cdr lib)))))
	    (error
	     (error "Can't get variables from require'd library %s"
		    name)))))
       ))
    env))
...
The environment created by the form is returned."
  (cond
   ...
   ((symbolp form)
    (if (elint-unbound-variable form env)
